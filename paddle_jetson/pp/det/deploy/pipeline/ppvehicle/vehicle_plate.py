...
def get_dir_path(path, n):
    for _ in range(n):
        path = os.path.dirname(path)
    return path


# parent_path = get_dir_path(os.path.abspath(__file__), 3)
# sys.path.insert(0, parent_path)
sys.path.append(get_dir_path(os.path.abspath(__file__), 5))

from paddle_jetson.pp.det.deploy.python.infer import get_test_images
from paddle_jetson.pp.det.deploy.python.preprocess import preprocess, NormalizeImage, Permute, Resize_Mult32
from paddle_jetson.pp.det.deploy.pipeline.ppvehicle.vehicle_plateutils import create_predictor, get_infer_gpuid, get_rotate_crop_image, draw_boxes
from paddle_jetson.pp.det.deploy.pipeline.ppvehicle.vehicleplate_postprocess import build_post_process
from paddle_jetson.pp.det.deploy.pipeline.cfg_utils import merge_cfg, print_arguments, argsparser
