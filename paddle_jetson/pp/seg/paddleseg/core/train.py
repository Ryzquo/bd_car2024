...
def train(model,
    ...
    while iter < iters and not stop_status:
        ...
        for data in loader:
            ...
            if num_classes == 2:
                labels = paddle.where((labels >= num_classes) & (labels != 255), paddle.ones_like(labels), labels)
            else:
                labels = paddle.where((labels >= num_classes) & (labels != 255), paddle.zeros_like(labels), labels)
            ...
            #     label_color[label_np == 0] = [0, 0, 255]    # 类别0为红色
            #     label_color[label_np == 1] = [0, 255, 0]    # 类别1为绿色
            #     label_color[label_np == 2] = [255, 0, 0]    # 类别2为蓝色
                
            #     # for class_idx in range(10):
            #     #     if class_idx != 1:
            #     #         label_color[label_np == class_idx] = colors[class_idx]
            
            # # 显示彩色标签图像
            # cv2.imshow(f"Label", label_color)
            # cv2.waitKey(0)  # 等待1毫秒，允许图像刷新
